...
export default {
  ...
  mounted() {
    ...
    form[1].value = store.state.selectedTemplates.name;
    form[2].value = store.state.selectedTemplates.subject;
      ...
      axios
        .put("api/campaign/template/get/message/"+store.state.selectedTemplates.id+"/",{})
        .then(function (response) {
            store.state.message = response['data']['message']
        })
        .catch(function (error) {
          if (error.response) {
            Swal.fire({
              title: "Error",
              text: "Failed to get template message",
              icon: "error",
    ...
    this.fv.on("core.form.valid", () => {
      ...
      axios.defaults.headers.common["Authorization"] =
        "Token " + Vue.$cookies.get("key");
      axios
        .put("api/campaign/template/manage/"+store.state.selectedTemplates.id+"/", data)
        .then(function () {
            store.state.TemplatesPageControls.table = true;
            store.state.TemplatesPageControls.update = false;
        })
        .catch(function (error) {
          if (error.response) {
            Swal.fire({
  ...
  computed: {
    editorData: {
        set(editorData){
            store.state.message = editorData;
        },
        get(){
